...
public class in_request extends JFrame {
	 ...
	 * Create the frame.
	 */
	public in_request(StudentInformationDTO info) {
		setTitle("입실 신청");
		setVisible(true);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(db.student_inandout_request(info, "입실")) {
					showConfirmationDialog("입실 신청이 완료되었습니다!");
					inandout_main main = new inandout_main(info);
					main.setVisible(true);
					dispose();
				}
				else {
					showConfirmationDialog("입실 신청 실패");
				}
			}
		});
		...
		JLabel lblNewLabel_3 = new JLabel("입실 신청서");
		...
		JLabel lblNewLabel_3_1 = new JLabel("*개인정보가 맞는지 확인 후 신청바람");
